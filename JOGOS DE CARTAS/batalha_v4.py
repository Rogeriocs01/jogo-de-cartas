...
class Jogador:
    ...
    def usar_habilidade(self, slot, oponente):
        ...
        if carta and self.mana >= carta.custo_habilidade and carta.efeito_habilidade:
            ...
            print(f"{self.nome} usa habilidade de {carta.nome}: {carta.habilidade}")
...
def habilidade_cura(jogador, oponente, slot):
    jogador.vida += 3
    print(f"{jogador.nome} curou 3 de vida!")

def habilidade_dano_area(jogador, oponente, slot):
    for i in range(4):
        if oponente.campo[i]:
            oponente.campo[i].defesa -= 2
            print(f"{oponente.campo[i].nome} levou 2 de dano em área!")
...
def criar_deck():
    return [
        ...
        Carta("Sacerdote Curador", 3, 1, 3, "Cura Mágica", 2, habilidade_cura),
        Carta("Mago de Fogo", 6, 4, 4, "Dano em Área", 3, habilidade_dano_area),
        Carta("Bomba Viva", 4, 3, 2, "Explosão na Morte", 2, None, efeito_morte_dano),
    ] * 3

def mostrar_estado(jogador1, jogador2):
    print("\n==================== STATUS DO CAMPO ====================")
    print(f"Vida {jogador1.nome}: {jogador1.vida} | Mana: {jogador1.mana}")
    print(f"Vida {jogador2.nome}: {jogador2.vida} | Mana: {jogador2.mana}\n")
